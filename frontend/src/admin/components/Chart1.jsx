...
export default function CustomAxisTicks(props) {
...
export function CustomAxisTicksControls({ onChange, sessionStoreValues }) {
  const [state, setState] = React.useState(sessionStoreValues ?? defaultState);

  const updateState = (nextValues) => {
          ...
          <tr>
            ...
            <td style={{ padding: '0 1ex' }}>
              <select
                ...
                value={state.scale}
                onChange={(event) =>
                  updateState({ scale: event.target.value })
                }
              >
